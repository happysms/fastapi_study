    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if request.url.path.endswith("/api"):
            ...
            if "Authorization" in request.headers.keys():
                request.state.user = await self.token_decode(access_token=request.headers.get)
                # 토큰 없음
            else:
                if "Authorization" not in request.headers.keys():
                    response = JSONResponse(status=401, content=dict(msg="AUTHORIZATION_REQUIRED"))
        else:
            # 템플릿 렌더링인 경우 쿠기에서 토큰 검사
            print(request.cookies)
            request.cookies["Authorization"] = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpZCI6NywiZW1haWwiOiJraHVtaW5zdW5nQGtodS5hYy5rciIsIm5hbWUiOm51bGwsInBob25lX251bWJlciI6bnVsbCwicHJvZmlsZV9pbWciOm51bGwsInNuc190eXBlIjpudWxsfQ.X4rM314pAOH7dmr2a6BlVl5w92jzrwZqN3Kq9HHrpRw"
